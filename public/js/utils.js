...
window.ExamApp.utils = {
  ...
  getStatusBadgeClass(status) {
    const map = {
      ...
      needs_correction: 'badge-needs-correction',
    };
  ...
  badgeHtml(status) {
    ...
    const icons = { pending: '◎', confirmed: '✓', needs_correction: '↩' };
    return `<span class="badge ${cls}">${icons[status] || ''} ${label}</span>`;
  },
  ...
  showToast(message, type = 'success', duration = 3500) {
    ...
    toast.innerHTML = `<span style="font-size:15px;font-weight:700">${iconMap[type] || '•'}</span><span>${message}</span>`;
